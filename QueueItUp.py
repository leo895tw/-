...
def assemble_queue():
	...
	if source_paths:
		outputname = f"{target_name}-{source_name}-{timestamp}"
	else:
		outputname = f"{target_name}-{timestamp}"
	queueitup_job_id = outputname
...
def edit_queue():
	...
	try:
		...
		close_button = tk.Button(root, text="Close Window 關閉視窗", command=on_window_close_internal, font=custom_font)
		close_button.pack(pady=5)

		settings_button2 = tk.Button(root, text="queueitup_settings 設定", command=queueitup_settings, font=custom_font)
		settings_button2.pack(pady=5)
		refresh_button = tk.Button(root, text="Refresh View 刷新檢視", command=refresh_frame_listbox, font=custom_font)
		refresh_button.pack(pady=5)

		run_jobs_button = tk.Button(root, text="Run Pending Jobs 執行待處理任務", command=run_jobs_click, font=custom_font)
		...
		missing_jobs_button = tk.Button(root, text="Delete Missing 刪除缺失任務", command=lambda: jobs_to_delete("missing"), font=custom_font)
		missing_jobs_button.pack(pady=5)

		archived_jobs_button = tk.Button(root, text="Delete archived 刪除已歸檔任務", command=lambda: jobs_to_delete("archived"), font=custom_font)
		archived_jobs_button.pack(pady=5)

		failed_jobs_button = tk.Button(root, text="Delete Failed 刪除失敗任務", command=lambda: jobs_to_delete("failed"), font=custom_font)
		failed_jobs_button.pack(pady=5)

		completed_jobs_button = tk.Button(root, text="Delete Completed 刪除已完成任務", command=lambda: jobs_to_delete("completed"), font=custom_font)
	...
	try:
		...
		if frame.winfo_exists():
			...
			for index, job in enumerate(jobs):
				...
				move_top_button = tk.Button(move_job_frame, text=" 移至最上方 Move to Top ", command=lambda idx=index, j=job: move_job_to_top(idx))
				move_top_button.pack(side='top', fill='y')
				move_up_button = tk.Button(move_job_frame, text=" 上移 Move Up ", command=lambda idx=index, j=job: move_job_up(idx))
				move_up_button.pack(side='top', fill='y')
				move_down_button = tk.Button(move_job_frame, text=" 下移 Move Down ", command=lambda idx=index, j=job: move_job_down(idx))
				move_down_button.pack(side='top', fill='y')
				move_bottom_button = tk.Button(move_job_frame, text=" 移至最下方 Move to Bottom", command=lambda idx=index, j=job: move_job_to_bottom(idx))
				...
				output_path_button = tk.Button(action_frame, text="輸出路徑 Output Path", command=lambda j=job: output_path_job(j))
				output_path_button.pack(side='top', padx=2)

				delete_button = tk.Button(action_frame, text=" 刪除 Delete ", command=lambda j=job: delete_job(j))
				delete_button.pack(side='top', padx=2)

				button_text = "取消歸檔 Un-Archive" if job['status'] == "archived" else "歸檔 Archive"
				...
				clone_job_button = tk.Button(action_frame, text="複製任務 Clone Job", command=lambda j=job: clone_job(j))
				clone_job_button.pack(side='top', padx=2)

				batch_button = tk.Button(action_frame, text="批次處理 BatchItUp", command=lambda j=job: batch_job(j))
				...
				argument_button = tk.Button(argument_frame, text="查看參數", wraplength=325, justify='center')
...
ADD_JOB_BUTTON = gradio.Button("新增任務 Add Job to QueueItUp", variant="primary")
RUN_JOBS_BUTTON = gradio.Button("執行 Run QueueItUp Jobs", variant="primary")
EDIT_JOB_BUTTON = gradio.Button("編輯 Edit QueueItUp Jobs")
SETTINGS_BUTTON = gradio.Button("更改設定 Change Settings")  # UNUSED: This button is defined but never used in the code
JOB_IS_RUNNING = 0
JOB_IS_EXECUTING = 0
CURRENT_JOB_NUMBER = 0
edit_queue_running = False
